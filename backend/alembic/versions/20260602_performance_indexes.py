...
Adds indexes that are frequently used but missing:
  - audit_logs: (target_type, target_id), (actor_type, actor_id), timestamp DESC
  ...
  - sigma_hits: (incident_id, severity) for analytics
...
def upgrade() -> None:
    ...
    # audit_logs — time-range queries (retention pruning, recent events); column is "timestamp"
    _safe_idx("ix_audit_logs_timestamp_desc", "audit_logs", "timestamp DESC")
